...
<body>
    <div class="container">
        ...
        <p>Halo {{ $data['name'] }},</p>
        <p>
			Terima kasih sudah melakukan pembayaran untuk reservasi paket {{ ucwords(strtolower($data['wahana']['name'])) }}. Nomor tiket kamu adalah #{{ $data['trans_num'] }}.
		</p>
        <p>
        ...
        <table>
            ...
            <tr>
                ...
                <td class="price">
                    {{ number_format($data['price']) }} x {{ $data['night_count'] }}<br>
        ...
        <p>
            Silahkan akses tiketmu <a href="{{ route('front_ticket', Crypt::encryptString($data['id'])) }}" target="_blank">disini</a> dan tunjukkan pada petugas saat check-in.
        </p>

        <p>
            Kami sungguh tidak ingin ini terjadi, tapi seandainya ada kekeliruan saat kamu memesan dan ingin membatalkan, silahkan <a href="{{ route('front_cancel', Crypt::encryptString($data['id'])) }}" target="_blank">kunjungi halaman ini.</a> <span class="cancel-link">(syarat dan ketentuan berlaku)</span>
        </p>

        <p>
            Sampai bertemu di {{ env('APP_NAME') }}. Kami menantikan kedatanganmu!
        </p>
</body>
</html>