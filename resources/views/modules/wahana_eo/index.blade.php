    ...
    <div class="card">
        <div class="card-header d-sm-flex align-items-sm-center py-sm-0">
            ...
            <div class="ms-sm-auto my-sm-auto">
                @can('eo-create')
                    <a href="{{ route('wahana.eo.create') }}" class="btn btn-primary btn-labeled btn-labeled-start">
...
<script>
    ...
    $(document).ready(function() {
        tableData = $('.datatable-basic').DataTable({
            ...
            columns: [
                ...
                { data: 'commission', name: 'commission', class: 'text-center', orderable: true, searchable: true, sortable: true },
                { data: 'commission_type', name: 'commission_type', class: 'text-center', orderable: true, searchable: true, sortable: true },
    ...
    function preaction(i){
        sw_delete_validated(
            "{{ route('wahana.eo.show', ':id') }}".replace(':id', i),
            "{{ csrf_token() }}",
            i,
            "{{ route('wahana.eo.destroy', ':id') }}".replace(':id', i),
            "{{ csrf_token() }}",
            "tableData",
            tableData
        );
