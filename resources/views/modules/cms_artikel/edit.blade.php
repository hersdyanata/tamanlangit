                ...
                <div class="card-body border-bottom border-light">
                    <form id="form_data">
                            ...
                            <div class="col-lg-10">
                                <select class="form-control select" data-minimum-results-for-search="Infinity" name="category_id" id="category_id">
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}" {{ ($category->id == $artikel->category_id) ? 'selected' : '' }}>{{ $category->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-10">
                                <textarea name="content" class="form-control" id="ckeditor_classic_prefilled">{{ $artikel->content }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-10">
                                <select class="form-control select" data-minimum-results-for-search="Infinity" name="status" id="status">
                                    ...
                                    <option value="published" {{ ($artikel->status == 'published' ? 'selected' : '') }}>Publish</option>
                                </select>
                            </div>
                        </div>
...
<script>
    ...
    function save(){
        $.ajax({
            type: "PUT",
            url: "{{ route('cms.artikel.update', $artikel->id) }}",
