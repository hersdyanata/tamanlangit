    ...
    <div class="card">
        <div class="card-header d-sm-flex align-items-sm-center py-sm-0">
            ...
            <div class="ms-sm-auto my-sm-auto">
                @can('user-create')
                    <a href="{{ route('acl.user.create') }}" class="btn btn-primary btn-labeled btn-labeled-start">
...
<script>
    ...
    $(document).ready(function() {
        tableData = $('.datatable-basic').DataTable({
            ...
            columns: [
                ...
                { data: 'granted_role', name: 'granted_role', orderable: true, searchable: true, sortable: true },
                {
                    data: 'created_at',
                    name: 'created_at',
                    searchable: false,
    ...
    function preaction(i){
        sw_delete_validated(
            "{{ route('acl.user.show', ':id') }}".replace(':id', i),
            "{{ csrf_token() }}",
            i,
            "{{ route('acl.user.destroy', ':id') }}".replace(':id', i),
            "{{ csrf_token() }}",
            "tableData",
            tableData
        );
