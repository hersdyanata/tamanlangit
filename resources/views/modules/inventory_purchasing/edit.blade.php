        ...
        <div class="col-xl-12">
            <div class="card">
                ...
                <div class="card-body border-bottom border-light">
                    <div class="alert border-0 alert-warning">
                        <i class="ph-warning-circle me-2"></i>
                        <span class="fw-semibold">Peringatan!</span> Perubahan ini akan berdampak pada stock inventory! Jika stock salah satu produk pada pembelian ini sudah dihapus maka akan ditambahkan kembali sesuai dengan quantity yang diinput!
                    </div>
                    <div class="row mb-3">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-10">
                            <label class="form-check">
                                <input type="checkbox" name="toggle_ppn" id="toggle_ppn" class="form-check-input form-check-input-primary" {{ ($data->ppn != null) ? 'checked' : '' }}>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-lg-3">
                            <input type="text" class="form-control" name="ppn" id="ppn" placeholder="Persentase PPn...." value="{{ $data->ppn }}">
                        </div>
                        <div class="col-lg-7">
                            <input type="text" class="form-control" name="ppn_amount" id="ppn_amount" placeholder="Nilai PPn akan diakumulasi otomatis..." value="{{ number_format($data->ppn_amount, 0, ',', '.') }}">
                        </div>
                    </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-lg-10">
                            <input type="text" class="form-control" name="total_amount" id="total_amount" placeholder="Tagihan setelah dikenakan PPn..." readonly value="{{ number_format($data->total_amount, 0, ',', '.') }}">
                            <input type="text" name="non_stock" id="non_stock" value="{{ $data->non_stock }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-10">
                            <label class="form-check">
                                <input type="checkbox" name="toggle_non_stock" id="toggle_non_stock" class="form-check-input form-check-input-primary" {{ ($data->non_stock == 'non stock') ? 'checked' : '' }} disabled>
                                <span class="form-check-label">
                                    <code>
                                        Perubahan ini tidak tidak dapat mengubah keputusan "Masuk Stock" atau "Tidak Masuk Stock".
                                    </code>
                                </span>
                            </label>
                        </div>
                        ...
                        <tbody id="cartTable">
                            @foreach ($data->items as $r)
                                <tr>
                                    ...
                                    <td>
                                        <input type="number" class="form-control" name="price[]" id="price_{{ $r->product_id }}" value="{{ $r->price }}">
                                        <input type="hidden" class="form-control" name="product_id[]" value="{{ $r->product_id }}" readonly>
                                    </td>
                                    <td class="text-center">
                                        <input type="hidden" class="form-control" name="old_quantity[]" value="{{ $r->quantity }}" readonly>
                                        <div class="input-group">
                                            ...
                                            <input type="number" class="form-control quantity" name="quantity[]" value="{{ $r->quantity }}" data-product-id="{{ $r->product_id }}" readonly>
                                    ...
                                    <td class="text-end"><input type="text" class="form-control text-end subtotal" name="subtotal[]" value="{{ number_format($r->subtotal, 0, ',', '.') }}" id="subtotal_{{ $r->product_id }}" readonly></td>
                                    <td class="text-center"><button type="button" class="btn btn-sm btn-danger" onclick="removeFromCart(this, {{ $r->product_id }})">Hapus</button></td>
                                </tr>
                            @endforeach
                        </tbody>
                        <tfoot>
...
<script>
    $(document).ready(function() {
        ...
        @foreach($data->items as $r)
            addedProductIds.add({{ $r->product_id }});
        @endforeach
        ...
        function applyPpn(){            
            // console.log( headerTotal.value.replace(/[^\d.-]/g, '').replace('.', '') );
        ...
        window.addToCart = function (productId) {
            ...
            cell7.innerHTML = '<button class="btn btn-danger" onclick="removeFromCart(this, '+ selectedRow.id +')">Hapus</button>';
    ...
    function save(){
        $.ajax({
            type: "PUT",
            url: "{{ route('inventory.purchasing.update', $data->id) }}",
