        ...
        <div class="card-header d-sm-flex align-items-sm-center py-sm-0">
            ...
            <div class="ms-sm-auto my-sm-auto">
                @can('inventory-produk-create')
                    <a href="{{ route('inventory.kategori-produk.create') }}" class="btn btn-primary btn-labeled btn-labeled-start">
...
<script>
    ...
    $(document).ready(function() {
        tableData = $('.datatable-basic').DataTable({
            ...
            ajax: {
                url: "{{ route('inventory.kategori-produk.index') }}",
            },
            serverSide: true,
            processing: true,
            columns: [
    ...
    function preaction(i){
        sw_delete_validated(
            "{{ route('inventory.kategori-produk.show', ':id') }}".replace(':id', i),
            "{{ csrf_token() }}",
            i,
            "{{ route('inventory.kategori-produk.destroy', ':id') }}".replace(':id', i),
            "{{ csrf_token() }}",
            "tableData",
            tableData
        );
